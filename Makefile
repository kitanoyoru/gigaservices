...
KIND_CLUSTER_NAME := kitanoyoru-gigaservices
...
db:
	...
	docker build -t kitanoyoru/gigaservices/db:latest --file ./platform/db/Dockerfile .
	$(KIND) load docker-image kitanoyoru/gigaservices/db:latest --name $(KIND_CLUSTER_NAME)
	$(KUBECTL_CMD) apply --filename ./platform/db/deployment.yaml

.PHONY: gateway
gateway:
	$(KUBECTL_CMD) delete deploy -n gateway --ignore-not-found app
	docker build -t kitanoyoru/gigaservices/gateway:latest --file ./apps/gateway/Dockerfile .
	$(KIND) load docker-image kitanoyoru/gigaservices/gateway:latest --name $(KIND_CLUSTER_NAME)
	$(KUBECTL_CMD) apply --filename ./apps/gateway/deployment.yaml

.PHONY: authority
authority:
	$(KUBECTL_CMD) delete deploy -n authority --ignore-not-found app
	docker build -t kitanoyoru/gigaservices/authority:latest --file ./apps/authority/Dockerfile .
	$(KIND) load docker-image kitanoyoru/gigaservices/authority:latest --name $(KIND_CLUSTER_NAME)
	$(KUBECTL_CMD) apply --filename ./apps/authority/deployment.yaml

.PHONY: catalog
catalog:
	$(KUBECTL_CMD) delete deploy -n catalog --ignore-not-found app
	docker build -t mercari/mercari-microservices-example/catalog:latest --file ./apps/catalog/Dockerfile .
	$(KIND) load docker-image kitanoyoru/gigaservices/catalog:latest --name $(KIND_CLUSTER_NAME)
	$(KUBECTL_CMD) apply --filename ./apps/catalog/deployment.yaml

.PHONY: customer
customer:
	$(KUBECTL_CMD) delete deploy -n customer --ignore-not-found app
	docker build -t kitanoyoru/gigaservices/customer:latest --file ./apps/customer/Dockerfile .
	$(KIND) load docker-image kitanoyoru/gigaservices/customer:latest --name $(KIND_CLUSTER_NAME)
	$(KUBECTL_CMD) apply --filename ./apps/customer/deployment.yaml

.PHONY: item
item:
	$(KUBECTL_CMD) delete deploy -n item --ignore-not-found app
	docker build -t kitanoyoru/gigaservices/item:latest --file ./apps/item/Dockerfile .
	$(KIND) load docker-image kitanoyoru/gigaservices/item:latest --name $(KIND_CLUSTER_NAME)
	$(KUBECTL_CMD) apply --filename ./apps/item/deployment.yaml

.PHONY: pb
pb:
	docker run \
		--volume "$(shell pwd):/go/src/github.com/mercari/mercari-microservices-example" \
		--workdir /go/src/github.com/kitanoyoru/gigaservices \
		--rm \
